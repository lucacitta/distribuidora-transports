...
// Definición de las cuatro pestañas y sus columnas.
export const TABS = {
  libreta: {
    title: "Libreta",
  ...
  agenda: {
    title: "Agenda",
    headers: ["id", "fecha", "chofer", "tipo", "nombre", "barrio", "direccion", "carga", "retira", "horario", "transporte", "notas", "impKey", "recurrenteId", "impFechaSug", "impChoferSug"],
  },
  hojas: {
    title: "Hojas de ruta",
  ...
  recurrentes: {
    title: "Recurrentes",
    headers: ["id", "nombre", "tipo", "direccion", "barrio", "transporte", "horario", "chofer", "dias"],
  },
};
...
export async function readAgenda() {
  ...
  return rows
    .map((r) => ({
      ...
      notas: r.get("notas") || "",
      impKey: r.get("impKey") || "",
      recurrenteId: r.get("recurrenteId") || "",
      impFechaSug: r.get("impFechaSug") || "",
      impChoferSug: r.get("impChoferSug") || "",
    }))
    .filter((p) => p.id);
}
...
export async function replaceAgenda(paradas) {
  ...
  const rows = (paradas || []).map((p) => ({
    ...
    impFechaSug: p.impFechaSug || "",
    impChoferSug: p.impChoferSug || "",
  }));
  if (rows.length) await sheet.addRows(rows);
  return { count: rows.length };
}
...
// ---------- Recurrentes (templates de viajes que se repiten) ----------

export async function readRecurrentes() {
  const doc = await getDoc();
  const sheet = await getSheet(doc, TABS.recurrentes);
  const rows = await sheet.getRows();
  return rows
    .map((r) => ({
      id: r.get("id") || "",
...
export async function replaceRecurrentes(recurrentes) {
  const doc = await getDoc();
  const sheet = await getSheet(doc, TABS.recurrentes);
  await sheet.clearRows();
